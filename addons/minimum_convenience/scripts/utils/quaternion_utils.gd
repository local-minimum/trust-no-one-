...
## Creates a tweener method using inital positions without rolling
static func create_tween_static_lookat_method_non_rolling(node: Node3D, target: Node3D, target_offset: Vector3, left: Vector3 = Vector3.LEFT) -> Callable:
    var start_rotation: Quaternion = node.global_basis.get_rotation_quaternion()
    var forward: Vector3 = (target.global_position - target.to_global(target_offset)).normalized()
    var up: Vector3 = forward.cross(left)
    var look_at: Quaternion = Basis.looking_at(forward, up).get_rotation_quaternion()

    return func (progress: float) -> void:
        var rot: Quaternion = lerp(start_rotation, look_at, progress)
        node.global_rotation = rot.get_euler()

## Creates a tweener method that dynamically tracks a moving target without rolling
static func create_tween_dynamic_lookat_method_non_rolling(node: Node3D, target: Node3D, left: Vector3 = Vector3.LEFT) -> Callable:
    var start_rotation: Quaternion = node.global_basis.get_rotation_quaternion()

    return func (progress: float) -> void:
        var forward: Vector3 = (target.global_position - node.global_position).normalized()
        var up: Vector3 = forward.cross(left)
        var look_at: Quaternion = Basis.looking_at(forward, up).get_rotation_quaternion()
        var rot: Quaternion = lerp(start_rotation, look_at, progress)
        node.global_rotation = rot.get_euler()

## Creates a tweener method that dynamically tracks a moving target and maintains a global up
static func create_tween_dynamic_lookat_method(node: Node3D, target: Node3D, up: Vector3 = Vector3.UP) -> Callable:
    var start_rotation: Quaternion = node.global_basis.get_rotation_quaternion()
