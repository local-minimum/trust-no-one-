...
static func safe_getv3(dict: Dictionary, key: String, default: Vector3 = Vector3.ZERO, warn: bool = true) -> Vector3:
    if dict.has(key):
        if dict[key] is Vector3:
            return dict[key]
        elif warn:
            push_warning("Dictionary %s has %s on key %s, expected an vector 3" % [dict, dict[key], key])
    elif warn:
        push_warning("Dictionary %s lacks key %s" % [dict, key])
